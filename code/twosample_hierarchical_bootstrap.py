...
def twosample_hierarchical_bootstrap(df, variable, condition, level_1, level_2,
    ...
    p-value close to 0 indicates that distributions_0 is greater than 
    distributions_1; and a p-value close to 1 indicates that distributions_1 is 
    greater than distributions_0.
    ...
    **kwargs : dict
    ...
    distribution_0 = hierarchical_resampling(df_0, variable, level_1, level_2, 
                                             n_iterations)
    distribution_1 = hierarchical_resampling(df_1, variable, level_1, level_2, 
                                             n_iterations)
...
def _compute_p_value(distribution_0, distribution_1, n_bins=30):    
    ...
    (https://github.com/soberlab/Hierarchical-Bootstrap-Paper)
    '''
...
def _plot_results(df, variable, condition, distribution_0, 
                            distribution_1, joint_prob, bin_edges, 
                            colors=['k','b'], labels=['0', '1']):
    """
    ...
    bin_edges_ = np.linspace(np.nanmin([data_0, data_1]), 
                             np.nanmax([data_0, data_1]), 30)
    ax0.hist(data_0.ravel(), bins=bin_edges_, color=colors[0], alpha=0.5, 
             label=labels[0])
    ax0.hist(data_1.ravel(), bins=bin_edges_, color=colors[1], alpha=0.5, 
             label=labels[1])
